...
public class Map implements Sprite {
  ...
  int clickDuration = 0;
  Image[] dmgmgs = {
      tk.getImage(DOCPATH + "dmg\\1.png"),
      tk.getImage(DOCPATH + "dmg\\2.png"),
      tk.getImage(DOCPATH + "dmg\\3.png"),
      tk.getImage(DOCPATH + "dmg\\4.png"),
      tk.getImage(DOCPATH + "dmg\\5.png"),
      tk.getImage(DOCPATH + "dmg\\6.png"),
      tk.getImage(DOCPATH + "dmg\\7.png"),
      tk.getImage(DOCPATH + "dmg\\8.png"),
      tk.getImage(DOCPATH + "dmg\\9.png"),
      tk.getImage(DOCPATH + "dmg\\10.png")
  };
  ...
  public Map(String fileloc, int type) {
    //type indicates if your using a old map and new char, or new map and old char
  }
  ...
  public void draw(Graphics g) {
    ...
    for (int loadx = p1.getLoadXMin(); loadx < p1.getLoadXMax(); loadx++) {
      for (int loady = p1.getLoadYMin(); loady < p1.getLoadYMax(); loady++) {
        ...
        if (loadx == clickx && loady == clicky && isPressingLeft && p1.selectedItemIsTool()
            && blocks[clickx][clicky].exists) {
          int percentBroke = (int) (10.0 * ((double) timer / p1.getPickStr()));
          g.drawImage(dmgmgs[percentBroke - 1],
              WIDTH / 2 + ((loadx - (length / 2)) * blockSize) - p1.getX(),
              HEIGHT / 2 + ((loady - (height / 2)) * blockSize) + p1.getY(), BLOCKSIZE, BLOCKSIZE,
              null);
        }
    ...
    for (NPC anEn : en) {
      anEn.draw(g, p1.getX(), p1.getY(), length);
    }
    p1.draw(g);

    for (int loadx = p1.getLoadXMin(); loadx < p1.getLoadXMax(); loadx++) {
      for (int loady = p1.getLoadYMin(); loady < p1.getLoadYMax(); loady++) {
        shading[loadx][loady].draw(g, WIDTH / 2 + ((loadx - (length / 2)) * blockSize) - p1.getX(),
            HEIGHT / 2 + ((loady - (height / 2)) * blockSize) + p1.getY());
      }
    }
  ...
  public boolean isNotStandable(int x, int y) {
    return blocks[x][y].isNotStandable();
  }
  ...
  public int getRealMouseX(int x) {
    return (int) (p1.getRealx() - ((WIDTH / 2) / blockSize) + (x / blockSize));
  }


  public int getRealMouseY(int y) {
    return (int) (p1.getRealy() - ((HEIGHT / 2) / blockSize) + (y / blockSize) + 1);
  }
  ...
  public void spreadLight(int currentx, int currenty, double lastLight) {
    ...
    if (newLight <= shading[currentx][currenty].getLightlvl()) {
      return;
    }
